...
/**
 * Sync ventes Tiller -> stock cave (production, v2).
 ...
 *  - Écrit par défaut (appelé par le cron). ?dryrun=1 pour simuler. ?days=N fenêtre.
 */
...
async function pagedAll(table: string, select: string) {
  const out: any[] = []; let from = 0; const SZ = 1000;
  while (true) {
    const { data, error } = await supabase.from(table).select(select).range(from, from + SZ - 1);
...
serve(async (req) => {
  ...
  const live = url.searchParams.get("dryrun") !== "1";   // écrit par défaut
  ...
  try {
    ...
    const movsAll = await pagedAll("cave_stock_movements", "wine_id,created_at,type,commentaire");
    ...
    for (const mv of movsAll) {
      ...
      if (mv.commentaire?.startsWith?.("tiller:")) synced.add(mv.commentaire.slice(7));
    }
    ...
    for (const s of sales) {
      ...
      if (cut && s.created_at < cut) { skippedCutoff++; continue; }
    ...
    return new Response(JSON.stringify({
      mode: live ? "LIVE" : "SIMULATION", window_days: days, sales_scanned: sales.length,
      already_synced: alreadySynced, decremented: live ? inserted : toInsert.length,
      skipped_before_inventory: skippedCutoff, unmatched_products: unmatchedList.length,
      unmatched_top: unmatchedList.slice(0, 15), errors: errors.length ? errors.slice(0, 10) : undefined,
    }, null, 2), { headers: cors });
  } catch (e) {
